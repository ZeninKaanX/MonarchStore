...
- Güncellenen GitHub Pages yardımcı betiği `localFallbackHash` ve Web Crypto kontrolünü canlı olarak sunuyor. Mobil önizlemede **Hesap oluştur** düğmesi başlıkta görünür, okunaklı ve ulaşılabilir durumdadır.
- HTTP uyumluluk sürümü yayımlandıktan sonra önbellek atlayan canlı URL’de hesap penceresi yeniden açıldı; kayıt akışının tamamlanması için geçici test hesabı oluşturma denemesi devam ediyor.
- HTTP uyumluluk kodu yayımlanmış olsa da tarayıcıdaki önceden yüklenmiş `account-local.js` modülü eski sürümde kaldı ve ikinci denemede aynı `crypto.subtle` hatası gözlendi. Kök HTML’de modül URL’sine sürüm parametresi eklenerek bu modül önbelleği geçersiz kılındı.
